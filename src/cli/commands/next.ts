/**
 * `viberaci next` -- what do I do right now?
 *
...
export function runNext(project: OpenedProject, options: NextOptions = {}): void {
  ...
  if (isComplete(plan, progress)) {
    say();
    ok(s.allDone);
    say(dim("  viberaci status  shows everything you did."));
    say();
    return;
  }
  ...
  if (step.kind === "human" || !step.roleId) {
    ...
    if (options.show) {
      ...
      numberedLine(2, s.copyPromptFrom(promptPath(step)));
      say(`     ${dim(s.orCopyToClipboard(cyan("viberaci next --copy")))}`);
    }
  }
  ...
  say(`  ${dim(s.thenRun)}  ${cyan("viberaci done")}`);
  say();
}
