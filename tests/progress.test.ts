...
let dir: string;
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), "viberaci-progress-"));
});
afterEach(() => {
...
describe("the progress file", () => {
  ...
  it("starts fresh rather than crashing on a damaged file", () => {
    mkdirSync(join(dir, ".viberaci"), { recursive: true });
...
describe("the commands", () => {
  ...
  it("refuses politely outside a project, and says how to make one", () => {
    const elsewhere = mkdtempSync(join(tmpdir(), "viberaci-empty-"));
    try {
      expect(() => openProject(elsewhere)).toThrow(FriendlyError);
      expect(() => openProject(elsewhere)).toThrow(/not a vibeRACI project/);
    } finally {
  ...
  it("next shows the first step and where its prompt is", () => {
    ...
    expect(out).toContain("viberaci done");
  });
