...
let dir: string;
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), "vibesquad-test-"));
});
afterEach(() => {
...
describe("what a new project contains", () => {
  it("includes the runbook, the project brief, and the plan itself", () => {
    ...
    expect(paths).toContain(".vibesquad/plan.json");
  ...
  it("saves a plan that can be read back and re-validated", () => {
    const saved = planWorkspace(plan).files.find((f) => f.path === ".vibesquad/plan.json");
